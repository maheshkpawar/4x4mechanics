...
DEFAULT_SERVICES = [
    ...
    {"id":3,"icon":"🛢️","name":"Oil Change","desc":"Premium engine oil and filter replacement recommended for your specific car model.","price":"Starting ₹399"},
    ...
    {"id":5,"icon":"🚗","name":"Suspension Repair","desc":"Shock absorbers, struts, springs and alignment correction for a smooth ride.","price":"Starting ₹999"},
    {"id":6,"icon":"❄️","name":"AC Service","desc":"AC gas refilling, compressor check, condenser cleaning and cooling restoration.","price":"Starting ₹699"},
    {"id":7,"icon":"🔋","name":"Battery Replacement","desc":"Battery testing, jump start and replacement with reliable branded batteries.","price":"Starting ₹2,499"},
    {"id":8,"icon":"⚖️","name":"Wheel Alignment","desc":"Computerized 4-wheel alignment and balancing for even tyre wear.","price":"Starting ₹599"},
    {"id":9,"icon":"🚿","name":"Car Washing","desc":"Full exterior wash, interior vacuum, dashboard wipe and tyre shine.","price":"Starting ₹299"},
    {"id":10,"icon":"🚐","name":"Pickup & Drop","desc":"We pick up your car and return it when service is done.","price":"Call for rates"},
]
...
DEFAULT_REVIEWS = [
    ...
    {"id":3,"name":"Ravi Patil","stars":5,"text":"Transparent billing, genuine parts, quick service. Finally found a garage I can fully trust.","loc":"Nashik, Hyundai Creta owner"},
    {"id":4,"name":"Snehal Joshi","stars":5,"text":"Got my Honda City full service done here. Very thorough work. Car drives like new!","loc":"Nashik, Honda City owner"},
    {"id":5,"name":"Mahesh Gaikwad","stars":4,"text":"Good service at reasonable rates. Wheel alignment and brake pads done perfectly.","loc":"Nashik, Maruti Baleno owner"},
    {"id":6,"name":"Kavita Sharma","stars":5,"text":"Used pickup and drop service — very prompt. Car returned spotless with all issues fixed!","loc":"Nashik, Hyundai Verna owner"},
]
...
def serve_css():
    from static_css import STYLES
    return Response(STYLES, mimetype="text/css")

@app.route("/scripts.js")
def serve_js():
    from static_js import SCRIPTS
...
def delete_booking():
    if not is_admin(): return jsonify({"success": False}), 403
    bid = int(request.get_json().get("id", 0))
    save_json(BOOKINGS_FILE, [b for b in get_bookings() if b["id"] != bid])
    return jsonify({"success": True})
...
def add_service():
    ...
    services.append({"id":new_id,"icon":data.get("icon","🔧"),"name":data.get("name",""),"desc":data.get("desc",""),"price":data.get("price","Call for price")})
...
def delete_service():
    if not is_admin(): return jsonify({"success": False}), 403
    sid = int(request.get_json().get("id", 0))
    save_json(SERVICES_FILE, [s for s in get_services() if s["id"] != sid])
    return jsonify({"success": True})
...
def add_review():
    ...
    reviews.insert(0, {"id":new_id,"name":data.get("name",""),"stars":int(data.get("stars",5)),"text":data.get("text",""),"loc":data.get("loc","Nashik")})
...
def delete_review():
    if not is_admin(): return jsonify({"success": False}), 403
    rid = int(request.get_json().get("id", 0))
    save_json(REVIEWS_FILE, [r for r in get_reviews() if r["id"] != rid])
    return jsonify({"success": True})
...
def save_settings():
    ...
    data = request.get_json()
    settings = get_settings()
    for key in ["business_name","owner","phone","address","hours","tagline"]:
        if data.get(key): settings[key] = data[key]
